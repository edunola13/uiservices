<?php
     ...
     */
    function definir_uri_aplicacion(){
        ...
        */
        ...
        if(isset($GLOBALS['internacionalizacion']['locales'])){
            ...
            foreach ($locales as $locale) {
                ...
                if($locale_uri == $locale){
                    //Cambio el locale por el correspondiente
                    $locale_actual= $locale;
                    //Le quito la parte de internacionalizacion a la uri actual
                    $uri_actual= substr($uri_actual, strlen($locale));
                    $uri_actual= trim($uri_actual, "/");
                    
                    $base_url_locale= $base_url_locale . $locale . "/";
        ...
        define("LOCALE_URI", $locale_actual);
    ...
    function mapea_url_actual($url){
        ...
        if(count($partes_url) >= count($partes_uri_actual)){
            ...
            for($i= 0; $i < count($partes_uri_actual); $i++){
     ...
     * @param string url
     */
    function redireccion_externa($url){
        header("Location:" . $url);
        //Detiene el flujo
        exit;
    }
    ...
    function set_estado_header($codigo = 200, $text = ''){
        ...
        if ($codigo == '' OR ! is_numeric($codigo)){
            mostrar_error("Error Estado HTTP", "El codigo de estado debe ser numerico");
	}
	...
	if (isset($estados[$codigo]) AND $text == ''){
        ...
        if ($text == ''){
            mostrar_error("Error Estado HTTP", 'No status text available.  Please check your status code number or supply your own message text.');
	}
        //Cargo el protocolo
