 ...
 * @author Usuario_2
 */
class UI_Component extends Componente{
    protected $config;
    
    public function __construct() {
        parent::__construct();
        $this->config= file_get_contents(PATHAPP . CONFIGURACION . "components.json");
    ...
    public function renderizar($parametros = NULL) {        
        if(isset($parametros["componente"])){
            //Imprime el componente con la confiuracion y componentes hijos correspondientes
            ...
            $componentesHijos= $parametros["hijos"];
            ...
            return $this->definicion($parametros["nombre"]);
        }
    }
    ...
    protected function definicion($nombre){
        ...
        $folderView= $this->config[$nombre];
        $file= file_get_contents(PATHAPP . "vista/componentes/" . $folderView . '/' . $nombre . ".html.twig");
    ...
    private function herencia($file){
        ...
        if($file[$i] == "{" && $file[$i + 1] == "%"){
            ...
            if($i !== FALSE && $file[$i] == " "){
                ...
                $padreFile= file_get_contents(PATHAPP . "vista/" . $archivo); 
